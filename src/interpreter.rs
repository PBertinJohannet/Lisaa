...
impl Interpreter {
    ...
    pub fn run(
        &mut self,
        programm: HashMap<String, FunctionDecl>,
    ...
    pub fn function_call(&self, func: &FunctionCall) -> Result<LiteralExpr, String> {
        if self.native_functions.contains_key(func.name()) {
            ...
            self.user_defined_func(
                func.name(),
                func.args().iter().map(|a| self.evaluate(a)).collect(),
            )
        }
    }
    ...
    pub fn user_defined_func(
        &self,
        name: &String,
        args_given: Vec<Result<LiteralExpr, String>>,
    ) -> Result<LiteralExpr, String> {
        let depth = self.scopes.borrow().last().unwrap().depth;
        self.scopes.borrow_mut().push(Scope::new(1));
        let actual_function = self.functions
            .get(name)
        ...
        for s in actual_function.scope() {
            let res = self.run_statement(s)?;
            match res {
                StatementResult::Return(l) => {
                    self.scopes.borrow_mut().pop();
                    return Ok(l);
                }
                StatementResult::Break => {
                    Err("can't break a function like that you silly".to_string())
                }
                _ => Ok(()),
            }?;
        }
    ...
    pub fn binary(&self, exp: &BinaryExpr) -> Result<LiteralExpr, String> {
        ...
        match exp.operator().get_type() {
            ...
            e => Err(format!(
                "operator {:?} can not be applied to binaary expression",
                e
            )),
        }
    }
